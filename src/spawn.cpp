...
bool Spawn::spawnMonsterWithBlock(uint32_t spawnId, MonsterType* mType, const Position& pos, Direction dir, bool startup /*= false*/)
{   
    if(mType->realName != "")
       mType->name = mType->realName;
    ...
    monster->setMasterPosition(pos, radius);
    monster->setDirection(dir);
    monster->setSkull(SKULL_BLOCKSPAWN);
		...
		if(spawnedMap.count(spawnId) == 0)
		{
			if(OTSYS_TIME() >= sb.lastSpawn + sb.interval)
			{
				if(findPlayer(sb.pos))
				{
					...
					//continue;
					spawnMonsterWithBlock(spawnId, sb.mType, sb.pos, sb.direction);
				}else
