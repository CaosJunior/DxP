...
ServiceManager* services)
{
	...
	std::cout << "PokeDG 1.0 by Taiger/Dudu" << std::endl;
	...
	std::cout << ">> Iniciando conexão com banco de dados" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	...
	std::cout << ">> Carregando vocações" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando vocações");
	#endif
	if(!Vocations::getInstance()->loadFromXml())
		startupErrorMessage("Não foi possível carregar as vocações!");
	...
	std::cout << ">> Criando o mapa e seus respawns..." << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Criando o mapa e seus respawns...");
	#endif
	...
	std::cout << ">> Estado do servidor: PvP" << std::endl;
	

	std::cout << ">> Inicializando os serviços..." << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Inicializando os serviços...");
	#endif
	...
	std::cout << std::endl << ">> Carregamento completo, servidor ONLINE!" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregamento completo, servidor ONLINE!");
	#endif
	g_game.setGameState(GAME_STATE_NORMAL);
